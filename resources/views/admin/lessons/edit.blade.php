@extends('layouts.app')
@section('title', 'Edit ' . $lesson->title . ' – Aref Academy')
...
<h1 class="mb-6 text-2xl font-bold">Edit Lesson</h1>

<form method="POST" action="{{ route('admin.lessons.update', $lesson) }}" enctype="multipart/form-data" class="card mb-8 max-w-2xl space-y-4">
    @csrf
    @method('PUT')
        ...
        <ul class="space-y-2 text-sm">
            @foreach($lesson->attachments as $attachment)
                <li class="flex items-center justify-between rounded-lg border border-gray-200 px-3 py-2 dark:border-gray-800">
                    ...
                    <form method="POST" action="{{ route('admin.attachments.destroy', $attachment) }}" onsubmit="return confirm('Delete this attachment?')">
                        @csrf @method('DELETE')
...
<div class="card mb-8 max-w-2xl">
    <div class="mb-3 flex items-center justify-between">
        <h2 class="font-semibold">Quizzes</h2>
        <a class="btn" href="{{ route('admin.lessons.quizzes.create', $lesson) }}">+ New Quiz</a>
    </div>
    <ul class="space-y-2 text-sm">
        @forelse($lesson->quizzes as $quiz)
            <li class="flex items-center justify-between rounded-lg border border-gray-200 px-3 py-2 dark:border-gray-800">
                <span>{{ $quiz->title }} <span class="text-xs text-gray-400">({{ $quiz->questions->count() }} questions · pass {{ $quiz->pass_score }}%)</span></span>
                <div class="flex gap-2">
                    <a class="btn-secondary" href="{{ route('admin.quizzes.edit', $quiz) }}">Edit</a>
        ...
        @empty
            <li class="text-gray-400">No quizzes yet.</li>
        @endforelse
    </ul>
</div>
...
<div class="card max-w-2xl">
        ...
        @forelse($lesson->assignments as $assignment)
            <li class="rounded-lg border border-gray-200 px-3 py-2 dark:border-gray-800" x-data="{ open: false }">
                <div class="flex items-center justify-between">
                    <span>{{ $assignment->title }} <span class="text-xs text-gray-400">(max {{ $assignment->max_score }}@if($assignment->deadline) · due {{ $assignment->deadline->format('Y-m-d') }}@endif)</span></span>
                    <div class="flex gap-2">
                        <button type="button" class="btn-secondary" @click="open = !open">Edit</button>
                ...
                <form x-show="open" x-cloak method="POST" action="{{ route('admin.assignments.update', $assignment) }}" class="mt-3 space-y-3 border-t border-gray-200 pt-3 dark:border-gray-800">
                    @csrf @method('PUT')
                    <input class="input" name="title" value="{{ $assignment->title }}" required>
                    <textarea class="input" name="description" rows="2">{{ $assignment->description }}</textarea>
    ...
    <form method="POST" action="{{ route('admin.lessons.assignments.store', $lesson) }}" class="space-y-3 border-t border-gray-200 pt-4 dark:border-gray-800">
        ...
        <textarea class="input" name="description" rows="2" placeholder="Instructions (optional)"></textarea>
        <div class="grid grid-cols-2 gap-3">
            <input class="input" name="max_score" type="number" min="1" value="100" placeholder="Max score" required>
